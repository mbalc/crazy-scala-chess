...
import crazyscalachess.movement.{MockMovementStrategy, MovementStrategy, Position}
...
class PieceTest extends FlatSpec with Matchers {
  ...
  it should "list all possible moves from its strategies" in {
    ...
    val Seq(strat1, strat2) = Seq(pos1, pos2).map(posList => new MockMovementStrategy(posList))
    val piece = new Piece(new Position(board, (5, 5)), fakeTeam, List[MovementStrategy](strat1, strat2))
  ...
  it should "not be able to move if there are no move strategies" in {
    val piece = new Piece(new Position(board, (5, 5)), fakeTeam, List[MovementStrategy]())
  ...
  it should "not be able to move if move strategies don't return anything" in {
    ...
    val piece = new Piece(new Position(board, (5, 5)), fakeTeam, List[MovementStrategy](emptyStrat, emptyStrat, emptyStrat))
    assert(piece.possibleMoves == List[Coordinates]())
  }
}
