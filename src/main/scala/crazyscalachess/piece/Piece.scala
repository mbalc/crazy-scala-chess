...
class Piece(val position: Option[Position], val team: Team, val movementStrategies: List[MovementStrategy]) {
  def this(position: Option[Position], team: Team, movementStrategy: MovementStrategy)
    = this(position, team, movementStrategy :: Nil)

  def this(position: Position, team: Team, movementStrategies: List[MovementStrategy])
