...
class Board (var whiteTeam: Team = null, var blackTeam: Team = null) {
  ...
  = (1 to columnCount).contains(coordinates _1) && (1 to rowCount).contains(coordinates _2)

  def getCoordinateContent(coordinates: Coordinates): Option[Piece] = {
    (whiteTeam getCoordinateContent coordinates, blackTeam getCoordinateContent coordinates) match {
